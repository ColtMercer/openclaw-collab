...
type MonthlyBarDatum = { _id: string; income: number; expenses: number };
...
export function CategoryDonut({ data }: { data: CategoryDatum[] }) {
  ...
  const items = other.length
    ? [...top, { _id: "Other", total: other.reduce((s, d) => s + d.total, 0) }]
    : top;
...
export function NetWorthLine({ data }: { data: NetWorthDatum[] }) {
  return (
    <ResponsiveContainer width="100%" height={300}>
      <LineChart data={data}>
...
export function DowChart({ data }: { data: DowDatum[] }) {
  ...
  const mapped = data.map((d) => ({ ...d, day: days[Number(d._id)] || d._id, avg: d.count ? d.total / d.count : 0 }));
  return (
    <ResponsiveContainer width="100%" height={300}>

...
export default async function DuplicateChargesPage() {
  ...
  const totalCharged = rows.reduce((sum, row) => sum + Math.abs(row.totalCharged || Math.abs(row.amount) * row.count), 0);
  const potentialSavings = rows.reduce((sum, row) => {
    const baselineCharge = Math.min(Math.abs(row.amount), Math.abs(row.totalCharged));
    return sum + Math.max(0, Math.abs(row.totalCharged) - baselineCharge);
  }, 0);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-1">
            Same merchant, same amount (±5%) within 48 hours.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <Card
          ...
          value={`${totalGroups}`}
          subtitle="48-hour duplicate clusters"
        ...
        <Card
          ...
          subtitle="Duplicate overcharges"
          className={potentialSavings > 0 ? "border-emerald-500/30" : ""}
        />
        <Card
          title="Total Charged"
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-zinc-500 text-left border-b border-zinc-800">
                ...
                <th className="pb-2 font-medium text-right">Window</th>
              ...
              {rows.map((row, i) => (
                <tr key={`${row.description}-${row.amount}-${i}`} className="border-b border-zinc-800/40 hover:bg-zinc-800/20">
                  ...
                  <td className="py-2 text-right font-mono font-semibold text-zinc-100">
                    {formatCurrency(Math.abs(row.totalCharged || Math.abs(row.amount) * row.count))}
                  </td>
                  <td className="py-2 text-right">
                    <span className="inline-flex items-center justify-center px-2 py-0.5 rounded-full text-xs font-semibold bg-indigo-500/15 text-indigo-300 border border-indigo-500/30">
                      within {row.windowHours}h
                    </span>
                  </td>
                  <td className="py-2 text-right">
